...
export const translations = {
  ...
  advertisersPage: {
    ...
    whyParas: {
      ru: [
        "Подключаем только проверенных веб-мастеров — каждый источник проходит проверку до старта трафика.",
        "Следим за качеством в режиме реального времени: слабые источники отключаем сами, до ваших претензий.",
        "Все спорные ситуации решаем быстро — прозрачная позиция по невалиду без затяжных разбирательств.",
        "Работаем на долгосрочный результат: масштабируем только то, что реально конвертит на вашем продукте.",
        "Менеджер погружён в ваш продукт и подбирает источники под конкретные KPI, а не просто закрывает объём.",
        "В итоге вы получаете управляемый поток с понятной экономикой — и партнёра, который отвечает за результат.",
      ],
      en: [
        "We connect only verified webmasters — each source is checked before traffic launch.",
        "We monitor quality in real time: we disconnect weak sources ourselves, before your complaints.",
        "All disputes are resolved quickly — a transparent stance on invalid traffic without prolonged proceedings.",
        "We work for long-term results: we scale only what actually converts on your product.",
        "The manager is immersed in your product and selects sources for specific KPIs, not just filling volume.",
        "In the end, you get a manageable flow with a clear economy — and a partner who is responsible for results.",
      ],
    },
